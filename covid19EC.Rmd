...
```{r, echo=FALSE}
#' https://cdn4.iconfinder.com/data/icons/coronavirus-glyph/64/coronavirus-sars-mers-flu-virus-influenza-512.png
htmltools::img(src = 'https://www.salud.gob.ec/wp-content/uploads/2020/03/img21.jpg ',
               alt = 'logo', 
...
g5 = ggplot(casos_diarios, aes(x = fecha, y = casos, fill = estado)) +
  geom_col(
    aes(text = paste0('Fecha: ', fecha, '<br>', estado, ': ', casos)),
    position = 'dodge2'
  ) +
...
msp = read.csv('https://raw.githubusercontent.com/pablora19/COVID19_EC/master/age%20range%20-%20alive_deads.csv', encoding = 'UTF-8') %>% 
  ...
  mutate(
    Clasificacion = ifelse(Estado == 'probablemente.fallecido', 'Probable', 'Confirmado')
  ) %>% 
  rename('Rango de Edad' = rango.edad) %>% 
  mutate(Estado = ifelse(Estado == 'probablemente.fallecido', 'Fallecido', Estado))
...
g3 = ggplot(msp2, aes(y = casos, x = `Rango de Edad`, fill = `Rango de Edad`)) + 
  geom_col(aes(frame = Fecha), show.legend = F,
